...
function loadCommands(bot, appConfig) {
    ...
    try {
            ...
            if (Array.isArray(commandList)) {
                for (const command of commandList) {
                    if (command.name && command.execute && command.regex) {
                        ...
                        // Pass a shared context (db, appConfig) to all commands
        ...
        logger.error('CMD_LOADER', 'Failed to load commands', { error: error.message });
    }
}
...
async function loadConfig() {
    ...
    try {
        ...
        logger.error('APP_CONFIG', 'Failed to load settings from database.', { error: error.message });
        process.exit(1);
    }
}
