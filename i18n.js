...
/**
 * [جدید] کاراکترهای خاص برای MarkdownV2 را escape می‌کند.
...
const languages = {
    fa: {
        ...
        // <<<< CHANGE START >>>>
        // متن `greeting_user_pending` اصلاح شد تا از تابع `escapeMarkdownV2Internal` برای پارامتر `admin` استفاده کند.
        // همچنین، escape کردن کاراکتر '_' در خود رشته `admin` که در تغییرات قبلی بود حذف شد
        // تا از Double Escaping جلوگیری شود.
        ...
        // <<<< CHANGE START >>>>
        // متن `registrationSuccess` اصلاح شد تا از تابع `escapeMarkdownV2Internal` برای پارامتر `admin` استفاده کند.
        // همچنین، escape کردن کاراکتر '_' در خود رشته `admin` که در تغییرات قبلی بود حذف شد
        // تا از Double Escaping جلوگیری شود.
