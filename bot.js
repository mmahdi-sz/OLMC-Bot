...
const { connectRcon, getRconClient, disconnectRcon } = require('./core/rconManager.js');
...
// فرض می‌شود که این فایل ایجاد شده است
...
async function main() {
    ...
    async function setupRankListCron() {
        if (rankListCronJob) {
            ...
            rankListCronJob = null;
        }
        
        const interval = await db.getSetting('rank_list_interval_minutes');
        if (interval && parseInt(interval) > 0) {
            rankListCronJob = cron.schedule(`*/${interval} * * * *`, () => {
                sendRankList(bot, db).catch(err => {
                    logger.error('RANK_CRON', 'Error in scheduled rank list send', { error: err.message });
                });
            });
    ...
    bot.on('message', async (msg) => {
        try {
            ...
            logger.error(MODULE_NAME, 'Error handling message', { error: error.message, stack: error.stack });
        }
    });
...
process.on('SIGINT', async () => {
    logger.info(MODULE_NAME, 'Received SIGINT signal, shutting down gracefully...');
    try {
        await new Promise(resolve => setTimeout(resolve, 500)); 
        disconnectRcon();
        process.exit(0);
    } catch (error) {
        logger.error(MODULE_NAME, 'Error during SIGINT shutdown', { error: error.message });
        process.exit(1);
    }
});

process.on('SIGTERM', async () => {
    logger.info(MODULE_NAME, 'Received SIGTERM signal, shutting down gracefully...');
    try {
        await new Promise(resolve => setTimeout(resolve, 500)); 
        disconnectRcon();
        process.exit(0);
    } catch (error) {
        logger.error(MODULE_NAME, 'Error during SIGTERM shutdown', { error: error.message });
        process.exit(1);
    }
});